...
resource "aws_internet_gateway" "ajjuigw" {

  vpc_id = aws_vpc.ajju.id
  tags = {
    "Name" = "${var.vpc_name}-ajjuigw"
  }
}
...
resource "aws_route_table" "rt" {
  ...
  route {
    ...
    gateway_id = aws_internet_gateway.ajjuigw.id
...
resource "aws_security_group" "ajju-sg" {
  vpc_id      = aws_vpc.ajju.id
...
resource "aws_instance" "ajju-instance" {
count =1
  ami                         = "ami-0261755bbcb8c4a84"
  key_name                    = "ajay"
  instance_type               = "t2.micro"
  vpc_security_group_ids =  [aws_security_group.ajju-sg.id]
